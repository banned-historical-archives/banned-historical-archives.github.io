...
export function useAuthorFilterDialog(authors_all: string[], onChange: (author: string) => void) {
  ...
  const columns = useRef<GridColDef[] >([
    {
      ...
      minWidth: 350,
      flex: 1,
      renderCell: (params: GridRenderCellParams<{name: string, selected: boolean}>) => {
        return (
            <Chip
              sx={{ m: 0.5 }}
              key={params.row.name}
  ...
  const authors = useMemo(() => authors_all.map(i => ({name: i, selected: selected == i})), [authors_all, selected]);
  const AuthorDialog = (
