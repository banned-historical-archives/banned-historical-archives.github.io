...
init().then(async (AppDataSource) => {
  ...
  const comment = await AppDataSource.getRepository(
    Comment,
  ).createQueryBuilder();
  ...
  */
      ...
      for (let j = 0; j < data[i].pages.length; j++) {
        ...
        let obj = await comment
          .where('articleId = :articleId', {
            articleId: data[i].pages[j].articleId,
          })
          .andWhere(
            new Brackets((qb) => {
  ...
  for (const book of books) {
    ...
    const dirPath = join(
      __dirname,
      `../txt/${book.entity.name?.replace(/\//g, '').replace(/\*/g, '').slice(0, 250)}`,
    );
    for (const i of res) {
