...
(async () => {
    ...
    for (const prefix of (await fs.readdir(p)).filter(
      ...
      for (const book of await fs.readdir(join(p, prefix))) {
        ...
        const bookinfo = JSON.parse(
          (
            await fs.readFile(join(p, prefix, book, book + '.bookinfo'))
          ).toString(),
        );
        for (const prefix2 of prefix2_list) {
          for (const article_file of (
            await fs.readdir(join(p, prefix, book, prefix2))
          ).filter((x) => x.endsWith('.json'))) {
            const article_id = parse(article_file).name;
            const article = JSON.parse(
              (
                await fs.readFile(join(p, prefix, book, prefix2, article_file))
              ).toString(),
            );
            const tags = JSON.parse(
              (
                await fs.readFile(
                  join(p, prefix, book, prefix2, `${article_id}.tag`),
                )
              ).toString(),
  ...
  fs.writeFileSync(
    join(__dirname, '../catelog.json'),
    JSON.stringify(catelog_temp_to_catelog(catelog)),
  );
  fs.writeFileSync(
