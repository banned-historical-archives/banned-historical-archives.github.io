...
export async function parse(
  ...
  const title = '王洪文、张春桥在作战部汇报批林批孔运动情况时的插话';
...
张春桥说：你那个排列顺序是不对的，黄、吴、李、邱是第一位的，怎么把阎洪滏排在第一位。这不是排列问题，这是个大问题。你们这样对总参问题的估计要犯错误，大舰队厉害呀！然后是小舰队，第三才是阎洪滏。这不是小事情，我老是觉得你们不搞黄、吴、李、邱的，为什么阎仲川的问题查不下去，就是不重视。阖洪滏是个小走卒。排列不对是会犯错误的。
