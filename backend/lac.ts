...
export default async function lac(
  text: string,
  dict?: string,
): Promise<LACResult[]> {
  text = text.replace(/\n/g, '');
  if (dict) {
    writeFileSync(join(normalize(__dirname), '../paddle/temp/temp.dict'), dict);
  }
  writeFileSync(join(normalize(__dirname), '../paddle/temp/temp.txt'), text);
  ...
  const [words, types]: [string[], LACType[]] = eval(
    raw.split('\n').filter((i) => i[0] === '[')[0],
  );
  const res: {
    [key: string]: {
