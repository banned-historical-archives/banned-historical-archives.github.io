...
const temp: any[] = fs.readdirSync(join(__dirname, './books'), { encoding: 'utf-8' })

let judged: any[] = fs.readdirSync(join(__dirname, '../patch/articles'), { encoding: 'utf-8' })

const map = new Map()

temp.forEach((item: string) => {
    const name = item.replace('.ts', '.js')
    const data = fs.readFileSync(join(__dirname, './books', item), { encoding: 'utf-8' })
...
init()
    .then(async (AppDataSource) => {
        try {
            ...
            all_publications.forEach(item => {
                ...
                objs.push(new Line(item.id, item.name, item.author, internal, official))
            })
    ...
    .then(() => {
        ...
        temp.forEach(item => {
            try {
                if (item.parser_option.articles.length === map.get(item.entity.id)) {
                    status = '已校对'
                } else if (item.parser_option.articles.length !== map.get(item.entity.id) && map.get(item.entity.id) > 0) {
                    status = '正在校对'
                } else {
                    status = '未校对'
                }
        ...
        objs.forEach((item, i) => {
            objs[i] = `${ item.title }|${ item.author }|${ item.internal }|${ item.official }|${ item.status }`
        })
        const result = first + mid1 + '\n' + objs.join('\n') + '\n\n' + mid2 + last;
        fs.writeFileSync(dirPath, result)
    })
    .then(() => {
        const arr = fs.readdirSync(join(__dirname, './temp'), { encoding: 'utf-8' })
        arr.forEach(item => {
            const name = item.replace('.ts', '.js')
